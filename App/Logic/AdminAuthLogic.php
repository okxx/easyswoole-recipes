...
final class AdminAuthLogic
{
    ...
    {
        $user = AdminInfoModel::create()->where('account',$account)->get();
        if (!$user) {
     ...
     * @throws \Throwable
     */
    public function infoByAccount(string $account): array
    {
        $user = AdminInfoModel::create()->where('account',$account)->get();
        if (!$user) {
            throw new \Exception(Common::USER_NOTFOUND,Status::CODE_BAD_REQUEST);
    ...
    public function register($account,$password,$requestCode)
    {
        ...
        $code = 8888;
        //$code = $this->rds()->get($this->request()->getQueryParam('account'));
